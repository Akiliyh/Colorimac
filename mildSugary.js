const mildSugarySketch = (p) => {
  let parent = document.getElementById('mildSugary-container');
  ...
  function init() {
    ...
    speed = [];
    curColors = [];
    contrast = { hue: 50, saturation: 0, lightness: 0 }; // mildSugary contrast values

    container.addEventListener('scroll', p.onScroll);

    // to change palette style change "mildSugaryPalette" & "mildSugaryColor"
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // map rotation according to hue value -> warm moves fast and mildSugary moves slow
      // % 360° cause 0° and 360° = same
      let distance = Math.abs((hueValue % 360) - 180);
      distance = Math.min(distance, 360 - distance);
      speed[i] = p.map(distance, 0, 180, 0, 0.01);
      displayColorInfo(mildSugaryPalette, i + 1);
    }
  }

  p.setup = function () {
    let canvas = p.createCanvas(w, h, p.WEBGL);
    canvas.id("mildSugary");
    canvas.parent("mildSugary-container");
    p.colorMode(p.HSL);
    p.noStroke();
    init();
  };
